...
17. Register the optional settings website plugin (`WEB_SETTINGS_ENABLED=true`).
18. `await client.login(DISCORD_TOKEN)` inside a try/catch - a `DisallowedIntents` rejection (privileged intent requested without approval) is logged as an actionable misconfiguration and exits, rather than leaving the process alive but disconnected.
...
When enabled, the settings website also starts after `clientReady`; see
[`docs/subsystems/settings-website.md`](../subsystems/settings-website.md).

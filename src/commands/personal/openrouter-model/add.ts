...
import { activatePersonalOpenRouterModelForCapability } from "@/utils/provider/providerActivation";
  ...
  try {
    ...
    switch (result.status) {
      ...
      case "already_registered":
        ...
        return;
      case "registered": {
        const activationResult = await activatePersonalOpenRouterModelForCapability({
          ...
          await replyInfoEmbed(interaction, locale, {
            titleKey:
              activationResult.status === "missing_provider"
                ? "commands.personal.openrouter_models.add.missing_provider_title"
                : "general.errors.update_failed_title",
            descriptionKey:
              activationResult.status === "missing_provider"
                ? "commands.personal.openrouter_models.add.missing_provider_description"
    ...
    const context: ErrorContext = {
      ...
      metadata: {
        command: "personal openrouter-model add",
        guildId: interaction.guild?.id,
        executorDiscordId: interaction.user.id,
      },
    };
    await log.error("Error executing /personal openrouter-model add", error as Error, context);
    await replyInfoEmbed(interaction, locale, {
